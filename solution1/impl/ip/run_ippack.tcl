...
set Library     "hls"
set IPName      "test_direct_dma"
set Version     "1.0"
set DisplayName "Test_direct_dma"
set Revision    "1504182159"
set Description "An IP generated by Vivado HLS"
set Device      "zynq"
set Taxonomy    "/VIVADO_HLS_IP"

## Variables
set Top "test_direct_dma"
...
set Interfaces {
    ...
    ap_clk {
        ...
        buses "s_axi_AXILiteS read_mover_V_V write_mover_V_V in_stream_V_V out_stream_V_V"
    }
    ap_rst_n {
        type "reset"
        polarity "ACTIVE_LOW"
...
proc add_bus_interface {core name type mode} {
    ...
    switch -- $type {
        ...
        axi4lite {
            ...
            if {$mode == "master"} {
                if {![info exists ::AddressSpace]} {
                    set ::AddressSpace [ipx::add_address_space Data $core]
                    set_property range 4G $::AddressSpace
                    set_property width 32 $::AddressSpace
                }
                dict set properties master_address_space_ref Data
            }
            if {$mode == "slave"} {
...
if {$SubcoreFiles != ""} {
    ...
    foreach ipfile $SubcoreFiles {
        ...
        set rtlfiles [glob -nocomplain $prj.srcs/sources_1/ip/$ipname/*.mif]
        foreach rtl $rtlfiles {
            file copy $rtl $ipdir
        }
...
if {[llength $VerilogFiles] > 0} {
    ...
    foreach f [concat $IPFiles $ConstraintFiles $VerilogFiles] {
        ...
        if {[file ext $f] == ".xdc"} {
            set_property used_in {out_of_context synthesis implementation} $current_file
        }
    }
...
dict for {interface_name interface_detail} $Interfaces {
    ...
    switch -nocase -- $type {
        ...
        native_axim {
           ...
           if {$HasOffset==0} {
               lappend rtl_parameters [list TARGET_ADDR  "std_logic_vector" 0x00000000 "bitString" "generated" value_bit_string_length 32]
        ...
        ap_fifo {
            ...
            if {$SupportDataType} {
                dict for {key value} $ctype {
                    dict set layered_metadata $iptypes::vlnv $key [create_datatype $value]
                }
            }
            lappend bus_parameters [list LAYERED_METADATA $layered_metadata generated]
        ...
        clock {
            ...
            set result [dict filter $Interfaces script {name detail} {
                string equal [dict get $detail type] reset
            }]
            set port [lindex [dict keys $result] 0]
            if {$port != ""} {
                add_bus_parameter $current_bus_interface ASSOCIATED_RESET $port
            }
